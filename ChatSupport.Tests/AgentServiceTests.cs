...
public class AgentServiceTests
{
    ...
    [Fact]
    public async Task ConnectAsync_WhenSuccess_ShouldReturnAgent(){
        // Arrange
        var agentId = ObjectId.GenerateNewId();
        var agent = new Agent { IsOnline = false, Shift = AgentShift.Afternoon, Name = "Agent1" };
        _agentRepositoryMock.Setup(a => a.GetById(agentId)).Returns(agent);
        _dateTimeProviderMock.Setup(dp => dp.Now).Returns(new DateTime(2021, 1, 1, 15, 0, 0));
    ...
    [Fact]
    public async Task HandleAgentQueueMessageAsync_ShouldNack_WhenAnyException()
    {
        // Arrange
        var agentId = ObjectId.GenerateNewId();
        var sessionId = ObjectId.GenerateNewId();
        
        //_agentRepositoryMock should throw an exception
        _agentRepositoryMock.Setup(a => a.GetById(agentId)).Throws(new Exception());
    ...
    [Fact]
    public async Task HandleAgentQueueMessageAsync_WhenIModelIsNull_ThrowsArgumentNullException()
    {
        // Arrange
        var deliveryArgs = new BasicDeliverEventArgs { DeliveryTag = 1, Body = Encoding.UTF8.GetBytes("message content") };

        // Act
        async Task Act() => await _agentService.HandleAgentQueueMessageAsync(null, deliveryArgs, ObjectId.GenerateNewId());
